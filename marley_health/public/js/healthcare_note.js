frappe.provide("marley_health");

marley_health.ClinicalNotes = class ClinicalNotes {
	constructor(opts) {
		$.extend(this, opts);
	}
...
marley_health.Orders = class Orders {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		frappe.call({
			method: "marley_health.marley_health.doctype.patient_encounter.patient_encounter.get_encounter_details",
			args: {
				"doc": me.frm.doc,
			},
			callback: (r) => {
		...
		let _create_service_request = () => {
			var d = new frappe.ui.Dialog({
				...
				primary_action: function() {
					var data = d.get_values();
					frappe.call({
						method: "marley_health.marley_health.doctype.patient_encounter.patient_encounter.create_service_request_from_widget",
						args: {
	...
	async update_status (status_btn, doctype, status) {
		...
		if (status == "Cancel") {
			...
			() => {
				frappe.call({
					method: "marley_health.marley_health.doctype.patient_encounter.patient_encounter.cancel_request",
						freeze: true,
						args: {
							doctype: doctype,
							request: order_name
