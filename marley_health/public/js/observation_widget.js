frappe.provide("marley_health.ObservationWidget");

marley_health.ObservationWidget = class {
	constructor(opts) {
		$.extend(this, opts);
		this.init_widget();
	}
	...
	auth_observation (observation, status) {
		...
		if (status == "Approved") {
			frappe.confirm(__("Are you sure you want to authorise Observation <b>" + observation +"</b>"), function () {
				frappe.call({
					method: 'marley_health.marley_health.doctype.observation.observation.set_observation_status',
					args: {
...
var set_text_to_dialog = function(d) {
	if (d.get_value("template")) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.observation.observation.get_observation_result_template',
			args: {
