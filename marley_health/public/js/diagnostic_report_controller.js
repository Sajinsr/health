frappe.provide("marley_health.Diagnostic.DiagnosticReport");

marley_health.Diagnostic.DiagnosticReport = class DiagnosticReport {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		this.ObservationWidgets = []
		  frappe.call({
			method: "marley_health.marley_health.doctype.observation.observation.get_observation_details",
			args: {
	...
	create_widget(r) {
		...
		this.result = []
			for (let key in r.message[0]) {
				me.ObservationWidgets[key] = new marley_health.ObservationWidget({
	...
	save_action(func) {
		...
		if (func=="save") {
			frappe.call({
				method: "marley_health.marley_health.doctype.observation.observation.record_observation_result",
				args: {
					values: this.result
				},
				freeze: true,
