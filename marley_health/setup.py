...
data = {
	...
	"on_setup": "marley_health.setup.setup_healthcare",
}
...
def setup_healthcare():
	...
	from marley_health.regional.india.abdm.setup import setup as abdm_setup
...
def setup_domain():
	...
	"""
	domain = frappe.get_doc("Domain", "Marley Health")
	domain.setup_domain()
...
def create_default_root_service_units():
	from marley_health.marley_health.utils import create_healthcare_service_unit_tree_root
...
def remove_from_active_domains():
	...
	frappe.db.delete("Has Domain", {"domain": "Marley Health"})


def remove_portal_settings_menu_items():
	"""Remove menu items added in Portal Settings"""
	menu_items = frappe.get_hooks("standard_portal_menu_items", app_name="marley_health")
