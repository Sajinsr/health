...
let make_discharge_summary = function(frm) {
	frappe.model.open_mapped_doc({
		method: "marley_health.marley_health.doctype.inpatient_record.inpatient_record.make_discharge_summary",
		frm: frm
	})
};
...
let transfer_patient_dialog = function(frm) {
	...
	dialog.fields_dict['leave_from'].get_query = function(){
		return {
			query : 'marley_health.marley_health.doctype.inpatient_record.inpatient_record.get_leave_from',
				...
				.then(r => {
					...
					if (values.name) {
						...
						Proceed to Cancel?`,
							() => {
								frappe.call({
								method: "marley_health.marley_health.doctype.inpatient_record.inpatient_record.cancel_amend_treatment_counselling",
									args: {
										args: args,
...
var create_treatment_counselling = function(frm, args) {
	frappe.call({
		method: "marley_health.marley_health.doctype.inpatient_record.inpatient_record.create_treatment_counselling",
		args: {
			ip_order: args
		},
		freeze: true,
