...
frappe.ui.form.on('Service Request', {
	...
	make_clinical_procedure: function(frm) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.service_request.service_request.make_clinical_procedure',
	...
	make_lab_test: function(frm) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.service_request.service_request.make_lab_test',
			args: { service_request: frm.doc },
			freeze: true,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
	...
	make_therapy_session: function(frm) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.service_request.service_request.make_therapy_session',
			args: { service_request: frm.doc },
			freeze: true,
			callback: function(r) {
				var doclist = frappe.model.sync(r.message);
	...
	make_observation: function(frm) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.service_request.service_request.make_observation',
			args: { service_request: frm.doc },
			freeze: true,
			callback: function(r) {
				if (r.message) {
