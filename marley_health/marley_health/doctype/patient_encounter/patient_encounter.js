...
frappe.ui.form.on('Patient Encounter', {
	...
	refresh: function(frm) {
			...
			if (frm.doc.drug_prescription && frm.doc.inpatient_record && frm.doc.inpatient_status === "Admitted") {
				frm.add_custom_button(__('Inpatient Medication Order'), function() {
					frappe.model.open_mapped_doc({
						method: 'marley_health.marley_health.doctype.patient_encounter.patient_encounter.make_ip_medication_order',
						frm: frm
					});
				},__('Create'));
			}
		...
		if (frappe.meta.get_docfield('Drug Prescription', 'medication').in_list_view === 1) {
			frm.set_query('drug_code', 'drug_prescription', function(doc, cdt, cdn) {
				...
				if (row.medication) {
					return {
						query: 'marley_health.marley_health.doctype.patient_encounter.patient_encounter.get_medications_query',
	...
	set_patient_info: async function(frm) {
		if (frm.doc.patient) {
			let me = frm
			frappe.call({
				method: 'marley_health.marley_health.doctype.patient.patient.get_patient_detail',
				args: {
					patient: frm.doc.patient
				},
				callback: function(data) {
...
var schedule_inpatient = function(frm) {
	...
	var dialog = new frappe.ui.Dialog({
		...
		primary_action : function() {
			...
			frappe.call({
				method: 'marley_health.marley_health.doctype.inpatient_record.inpatient_record.schedule_inpatient',
				args: {
...
var schedule_discharge = function(frm) {
	var dialog = new frappe.ui.Dialog ({
		...
		primary_action : function() {
			...
			frappe.call ({
				method: 'marley_health.marley_health.doctype.inpatient_record.inpatient_record.schedule_discharge',
				args: {args},
				callback: function(data) {
					if(!data.exc){
						frm.reload_doc();
...
let create_nursing_tasks = function(frm) {
	const d = new frappe.ui.Dialog({
		...
		primary_action: () => {
			...
			frappe.call({
				method: 'marley_health.marley_health.doctype.nursing_task.nursing_task.create_nursing_tasks_from_template',
				args: {
...
let cancel_ip_order = function(frm) {
	...
	function(data) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.inpatient_record.inpatient_record.set_ip_order_cancelled',
			async: false,
			freeze: true,
			args: {
...
let create_service_request = function(frm) {
	frappe.call({
		method: "marley_health.marley_health.doctype.patient_encounter.patient_encounter.create_service_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
let create_medication_request = function(frm) {
	frappe.call({
		method: "marley_health.marley_health.doctype.patient_encounter.patient_encounter.create_medication_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
frappe.ui.form.on('Drug Prescription', {
	...
	medication:function(frm, cdt, cdn) {
		...
		frappe.call({
			method: "marley_health.marley_health.doctype.patient_encounter.patient_encounter.get_medications",
			freeze: true,
			args: {
...
var show_clinical_notes = async function(frm) {
	if (frm.doc.docstatus == 0 && frm.doc.patient) {
		const clinical_notes = new marley_health.ClinicalNotes({
			frm: frm,
...
var show_orders = async function(frm) {
	if (frm.doc.docstatus == 0 && frm.doc.patient) {
		const orders = new marley_health.Orders({
			frm: frm,
