...
frappe.ui.form.on('Patient Appointment', {
	...
	refresh: function(frm) {
		...
		frm.set_query('service_unit', function() {
			return {
				query: 'marley_health.controllers.queries.get_healthcare_service_units',
				filters: {
			...
			if (frm.doc.procedure_template) {
				frm.add_custom_button(__('Clinical Procedure'), function() {
					frappe.model.open_mapped_doc({
						method: 'marley_health.marley_health.doctype.clinical_procedure.clinical_procedure.make_procedure',
						frm: frm,
					});
				}, __('Create'));
				...
				frm.add_custom_button(__('Therapy Session'), function() {
					frappe.model.open_mapped_doc({
						method: 'marley_health.marley_health.doctype.therapy_session.therapy_session.create_therapy_session',
						frm: frm,
					})
				}, 'Create');
			} else {
				frm.add_custom_button(__('Patient Encounter'), function() {
					frappe.model.open_mapped_doc({
						method: 'marley_health.marley_health.doctype.patient_appointment.patient_appointment.make_encounter',
						frm: frm,
					});
				}, __('Create'));
			}
		...
		if (!frm.is_new() && !frm.doc.invoiced && frm.doc.status != "Cancelled") {
			frappe.db.get_single_value("Healthcare Settings", "show_payment_popup").then(async val => {
				let fee_validity = (await frappe.call(
					"marley_health.marley_health.doctype.fee_validity.fee_validity.get_fee_validity",
		...
		frm.page.set_primary_action(__('Book'), async function() {
			...
			if (!frm.is_new()) {
				await frappe.db.get_single_value("Healthcare Settings", "show_payment_popup").then(val => {
					frappe.call({
						method: "marley_health.marley_health.doctype.fee_validity.fee_validity.check_fee_validity",
	...
	set_payment_details: function(frm) {
		frappe.db.get_single_value('Healthcare Settings', 'show_payment_popup').then(val => {
			if (val) {
				frappe.call({
					method: 'marley_health.marley_health.utils.get_appointment_billing_item_and_rate',
					args: {
						doc: frm.doc
					},
					callback: function(data) {
	...
	toggle_payment_fields: function(frm) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.patient_appointment.patient_appointment.check_payment_reqd',
	...
	get_prescribed_therapies: function(frm) {
		if (frm.doc.patient) {
			frappe.call({
				method: "marley_health.marley_health.doctype.patient_appointment.patient_appointment.get_prescribed_therapies",
				args: { patient: frm.doc.patient },
				callback: function(r) {
					if (r.message) {
...
let check_and_set_availability = function(frm) {
						...
						frappe.call({
							...
							callback: (r) => {
								if (val && !r.message && !frm.doc.invoiced) {
									...
									frappe.call({
										method: "marley_health.marley_health.doctype.patient_appointment.patient_appointment.update_fee_validity",
										args: { "appointment": frm.doc }
									});
								}
							}
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('practitioner')) {
			...
			frappe.call({
				method: 'marley_health.marley_health.doctype.patient_appointment.patient_appointment.get_availability_data',
				args: {
...
let get_prescribed_procedure = function(frm) {
	if (frm.doc.patient) {
		frappe.call({
			method: 'marley_health.marley_health.doctype.patient_appointment.patient_appointment.get_procedure_prescribed',
			args: { patient: frm.doc.patient },
			callback: function(r) {
...
let update_status = function(frm, status) {
	...
	frappe.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			frappe.call({
				method: 'marley_health.marley_health.doctype.patient_appointment.patient_appointment.update_status',
	...
	function show_payment_dialog(frm, fields) {
		let d = new frappe.ui.Dialog({
			...
			primary_action: async function(values) {
				...
				frappe.call({
					method: "marley_health.marley_health.doctype.patient_appointment.patient_appointment.invoice_appointment",
					args: {
