...
frappe.ui.form.on('Clinical Procedure Template', {
	...
	refresh: function (frm) {
		...
		frm.set_query('staff_role', function () {
			return {
				filters: {
					'restrict_to_domain': 'Marley Health'
				}
			};
		});
	},
...
let change_template_code = function (doc) {
	let d = new frappe.ui.Dialog({
		...
		primary_action: function () {
			...
			if (values) {
				frappe.call({
					'method': 'marley_health.marley_health.doctype.clinical_procedure_template.clinical_procedure_template.change_item_code_from_template',
