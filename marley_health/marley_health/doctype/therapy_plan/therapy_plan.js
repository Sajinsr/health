...
frappe.ui.form.on('Therapy Plan', {
			...
			if (frm.doc.status != 'Completed') {
				...
				frm.add_custom_button(__('Therapy Session'), function() {
					frappe.prompt(fields, data => {
						frappe.call({
							method: 'marley_health.marley_health.doctype.therapy_plan.therapy_plan.make_therapy_session',
							args: {
	...
	make_sales_invoice: function(frm) {
		frappe.call({
			...
			method: 'marley_health.marley_health.doctype.therapy_plan.therapy_plan.make_sales_invoice',
			callback: function(r) {
