...
class PatientHistory {
	...
	setup_filters() {
		...
		frappe.xcall(
			'marley_health.marley_health.page.patient_history.patient_history.get_patient_history_doctypes'
		).then(document_types => {
	...
	setup_documents(document_types="", selected_date_range="") {
		...
		let me = this;
		frappe.call({
			'method': 'marley_health.marley_health.page.patient_history.patient_history.get_feed',
			args: filters,
			callback: function(r) {
				let data = r.message;
				if (data.length) {
		...
		this.page.main.on('click', '.btn-more', function() {
			...
			if (me.page.main.find('.'+docname).parent().find('.document-html').attr('data-fetched') == '1') {
				...
				if (doctype && docname) {
					...
					frappe.call({
						method: 'marley_health.marley_health.utils.render_doc_as_html',
						args: {
							doctype: doctype,
							docname: docname,
	...
	show_patient_vital_charts(btn_id, scale_unit, title) {
		...
		frappe.call({
			method: 'marley_health.marley_health.utils.get_patient_vitals',
			args: {
				patient: me.patient_id
			},
			callback: function(r) {
